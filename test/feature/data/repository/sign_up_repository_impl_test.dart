  ...
  group('signUp -', () {
    ...
    test(
      ...
      () async {
        // arrange
        when(
          mockRemoteDataSource.requestSignUp(email: tEmail, password: tPassword),
    ...
    test(
      'should return sign up failure when the call to remote data source is unsuccessful at signing up',
      () async {
        // arrange
        when(
          mockRemoteDataSource.requestSignUp(email: tEmail, password: tPassword),
        ).thenThrow(const SignUpException('EMAIL_EXISTS'));
        // act
        ...
        verifyNoMoreInteractions(mockRemoteDataSource);
        expect(result, equals(const Left<Failure, void>(SignUpFailure('EMAIL_EXISTS'))));
      },
    );
